 ...
 */
int ALLOCATION, HUGEPAGES, ACCESSES, RUNS, OFFSET, BUFFERSIZE=0, ALIGNMENT;
int TLB_MODE=0,NUM_FLUSHES=0,NUM_USES=0,FLUSH_MODE,USE_MODE,NUM_THREADS=0;
int SHARE_CPU,FLUSH_L1=0,FLUSH_L2=0,FLUSH_L3=0,NUM_RESULTS,TIMEOUT=0;
...
void bi_getinfo( bi_info * infostruct )
{
   ...
   infostruct->kerneldescription = bi_strdup( KERNEL_DESCRIPTION );
...
void* bi_init( int problemsizemax )
{
   ...
   mdp = (mydata_t*) calloc( 1,sizeof( mydata_t ));
   if ( mdp == 0 )
   {
   ...
   mdp->cpuinfo=(cpu_info_t*)_mm_malloc( sizeof( cpu_info_t ),ALIGNMENT);
   if ( mdp->cpuinfo == 0 )
   {
   ...
   /* perform hw_detection */
   init_cpuinfo(mdp->cpuinfo,1);
   ...
   if (ALLOCATION==ALLOC_GLOBAL)
   {
     ...
     {
       fprintf( stderr, "Error: Allocation of buffer failed 1\n" ); fflush( stderr );
   ...
   /* 
    * pagesize is needed for the TLB optimisation, which is only needed if hugepages are not used.
    * TODO: fix, implement TLB optimisation for Hugepages
    */
   /*if (HUGEPAGES==HUGEPAGES_OFF) mdp->pagesize=mdp->cpuinfo->pagesizes[0];
   if (HUGEPAGES==HUGEPAGES_ON) mdp->pagesize=mdp->cpuinfo->pagesizes[1];*/
   mdp->pagesize=mdp->cpuinfo->pagesizes[0];
   
   if ((TLB_MODE)&&(mdp->cpuinfo->tlblevels>=TLB_MODE))
   {
     ...
     if (mdp->tlb_sets)
     {
       mdp->tlb_collision_check_array=(unsigned long long*)_mm_malloc(sizeof(unsigned long long)*(mdp->tlb_size/mdp->tlb_sets),ALIGNMENT);  
     }
     
     if (mdp->tlb_size)
     {     
       mdp->tlb_tags=(unsigned long long*)_mm_malloc(sizeof(unsigned long long)*mdp->tlb_size,ALIGNMENT);
     }
  ...
  if (ALLOCATION==ALLOC_LOCAL)
  {
   ...
   if (HUGEPAGES==HUGEPAGES_ON)
   {
      ...
      alloc = malloc(BUFFERSIZE);
      write(fd, alloc, BUFFERSIZE);
      free(alloc);
      mdp->buffer=(char*) mmap(NULL,BUFFERSIZE,PROT_READ|PROT_WRITE,MAP_SHARED,fd,0);
      close(fd);unlink(filename);
   } 
   if ((mdp->buffer == 0)||(mdp->buffer == (void*) -1ULL))
   {
      fprintf( stderr, "Error: Allocation of buffer failed 2\n" ); fflush( stderr );
      if (HUGEPAGES==HUGEPAGES_ON) perror("mmap");
      exit( 127 );
   }
  } 
  ...
  if (TLB_MODE) printf("  using only %i pages (which fit in Level %i TLB) for latency-measurement\n",mdp->tlb_size,mdp->max_tlblevel);
...
int bi_entry( void* mdpv, int problemsize, double* results )
{
  ...
  /* results */
  double *tmp_results=malloc(mdp->num_threads*sizeof(double));

  /* check wether the pointer to store the results in is valid or not */
  if ( results == NULL ) return 1;

  /* one call measures latencies in cycles for all selected CPUs */
	_work(problemarray[problemsize-1],ALIGNMENT,OFFSET,ACCESSES,RUNS,mdp,&tmp_results);
  results[0] = problemarray[problemsize-1];
  ...
  for (k=0;k<NUM_RESULTS;k++)
  {
    ...
    {
      results[1+(j+2)*NUM_RESULTS+k]=mdp->papi_results[j];
    }
    #endif
  }
  free(tmp_results);
...
void bi_cleanup( void* mdpv )
{
   ...
   if ( mdp )
   {
     ...
     //if (mdp->tlb_restore_array!=NULL) _mm_free (mdp->tlb_restore_array);
...
void evaluate_environment(bi_info * info)
{
   ...
   else
   {
     ...
     //problemlist = bi_parselist(&(problemlistsize), p);fflush(stdout);
     bi_parselist(p);
   ...
   if ( p == 0 ) {errors++;sprintf(error_msg,"BENCHIT_KERNEL_RUNS not set");}
   else RUNS = atoi( p );
   ...
   else {printf ("papi disabled: %i\n",atoi(p));fflush(stdout);}
   #endif

   if ( errors > 0 ) {
      fprintf( stderr, "There's an environment variable not set or invalid!\n" );
      fprintf( stderr, "%s\n", error_msg);
      exit( 1 );
   }
