...
void *thread(void *threaddata)
{
  ...
  if(mydata->buffersize)
  {
    ...
    if (global_data->hugepages==HUGEPAGES_ON)
    {
      ...
      if (fd == -1)
      {
        fprintf( stderr, "Allocation of buffer failed 3\n" ); fflush( stderr );
        perror("open");
        exit( 127 );
      }
      if (fallocate(fd, 0, 0, mydata->buffersize) != 0)
	      perror("fallocate work.c");
      alloc = malloc(mydata->buffersize);
      write_amnt = write(fd, alloc , mydata->buffersize);
      free(alloc);
      if( write_amnt == -1 || write_amnt < mydata->buffersize) {
	      perror("Write amount");
      }
      printf("Write amount was %d and buffer size is %d\n", write_amnt, mydata->buffersize);

      printf("Mapping data with size %lu\n", mydata->buffersize);
      mydata->buffer=(char*) mmap(NULL,mydata->buffersize,PROT_READ|PROT_WRITE,MAP_SHARED,fd,0);
      if(mydata->buffer == MAP_FAILED){
	      printf("Error is %s\n", strerror(errno));
      }
      printf("addr is at %p\n", mydata->buffer);
    ...
    for (i=0;i<mydata->buffersize;i+=tmp2)
    {
	    ...
	    *(unsigned long long *)(mydata->buffer + i) = i;
    }
