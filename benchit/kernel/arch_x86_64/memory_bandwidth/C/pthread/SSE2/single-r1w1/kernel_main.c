...
int NUM_FLUSHES=0,NUM_USES=0,FLUSH_MODE,USE_MODE_R,USE_MODE_W,NUM_THREADS=0,SHARE_CPU;
int FLUSH_L1=0,FLUSH_L2=0,FLUSH_L3=0,ALIGNMENT,BURST_LENGTH,USE_DIRECTION;
int NUM_RESULTS,METHOD,LAYOUT,LOCAL_READ=0,LOCAL_WRITE=0,TIMEOUT=0;
double VALUE,FACTOR;
...
void bi_getinfo( bi_info * infostruct )
{
   ...
   infostruct->outlier_direction_upwards = _mm_malloc( infostruct->numfunctions * sizeof( int ) ,ALIGNMENT);
   if ( infostruct->outlier_direction_upwards == 0 )
   {
     fprintf( stderr, "Allocation of outlier direction failed.\n" ); fflush( stderr );
     exit( 127 );
   }
   *infostruct->outlier_direction_upwards=0;
   ...
   for ( j = 0; j < n_of_works; j++ )
   {
      ...
      {
         sprintf(buf,"bandwidth: CPU%llu - CPU%llu",cpu_bind[0],cpu_bind[k]);
         ...
         switch ( j )
        {
          ...
          default: // papi
            ...
            infostruct->outlier_direction_upwards[index] = 0;
...
int inline bi_entry( void* mdpv, int problemsize, double* results )
{
  ...
  _work(problemarray[problemsize-1],(mydata_t*)mdpv,&results);
  results[0] = (double)(problemarray[problemsize-1]);
  ...
  for (k=0;k<NUM_RESULTS;k++)
  {
    ...
    {
      results[1+(j+1)*NUM_RESULTS+k]=((mydata_t*)mdpv)->papi_results[j];
    }
  }
  #endif
  
  /* remove data from caches */
  /* !!! disabled as it more offten affects performance negatively by creating useless TLB entries */
  /*__asm__ __volatile__("clflush (%%rax);":: "a" ((unsigned long long)&(results[0])));
  for (k=0;k<NUM_RESULTS;k++)
  {
    __asm__ __volatile__("clflush (%%rax);":: "a" ((unsigned long long)&(results[k+1])));
    #ifdef USE_PAPI
    for (j=0;j<papi_num_counters;j++)
    {
      results[1+(j+1)*NUM_RESULTS+k]=mdp->papi_results[j];
      __asm__ __volatile__("clflush (%%rax);":: "a" ((unsigned long long)&(results[1+(j+1)*NUM_RESULTS+k])));
      __asm__ __volatile__("clflush (%%rax);":: "a" ((unsigned long long)&(mdp->papi_results[j])));
    }
    #endif    
  }
  __asm__ __volatile__("clflush (%%rax);":: "a" ((unsigned long long)&(problemarray[problemsize-1])));
  __asm__ __volatile__("clflush (%%rax);":: "a" ((unsigned long long)&j));
  __asm__ __volatile__("clflush (%%rax);":: "a" ((unsigned long long)&k));
  __asm__ __volatile__("clflush (%%rax);":: "a" ((unsigned long long)&problemsize));
  */
  return 0;
}
...
void evaluate_environment(bi_info * info)
{
   ...
   {
     if (!strcmp(p,"copy")) METHOD=METHOD_COPY;
     else if (!strcmp(p,"scale")) METHOD=METHOD_SCALE;
     else if (!strcmp(p,"independent")) METHOD=METHOD_INDEP;     
   ...
   if (p==0) {errors++;sprintf(error_msg,"BENCHIT_KERNEL_HUGEPAGE not set");}
   else
   {
     if (!strcmp(p,"0")) HUGEPAGES=HUGEPAGES_OFF;
     else if (!strcmp(p,"1")) HUGEPAGES=HUGEPAGES_ON;
     else {errors++;sprintf(error_msg,"invalid setting for BENCHIT_KERNEL_HUGEPAGE");}
   }
   ...
   else
   {
     ...
     else if (!strcmp(p,"mov+clflush")) {ALIGNMENT=64;OFFSET=OFFSET%ALIGNMENT;FUNCTION=USE_MOV_CLFLUSH;}
     else if (!strcmp(p,"movapd")) {ALIGNMENT=64;OFFSET=0;FUNCTION=USE_MOVAPD;}
     else if (!strcmp(p,"movupd")) {ALIGNMENT=64;OFFSET=OFFSET%ALIGNMENT;FUNCTION=USE_MOVUPD;}
     else if (!strcmp(p,"movntpd")) {ALIGNMENT=64;OFFSET=0;FUNCTION=USE_MOVNTPD;}
   ...
   if ((BURST_LENGTH==8)&&((FUNCTION==USE_MOV)||(FUNCTION==USE_MOVNTI)||(FUNCTION==USE_MOV_CLFLUSH)))
     {errors++;sprintf(error_msg,"BURST LENGTH 8 not supported for 64 Bit operations");}
     
   if ((BURST_LENGTH>4)&&(BURST_LENGTH!=8))
