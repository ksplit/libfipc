...
void bi_getinfo( bi_info * infostruct )
{
   ...
   infostruct->codesequence = bi_strdup( "movdqa mem -> xmm" );
   ...
   *infostruct->outlier_direction_upwards=0;
   ...
   for ( j = 0; j < n_of_works; j++ )
   {
      ...
      {
         if (k)  sprintf(buf,"read bandwidth CPU%llu accessing CPU%llu memory",cpu_bind[0],cpu_bind[k]);
           else sprintf(buf,"read bandwidth CPU%llu locally",cpu_bind[0]);
