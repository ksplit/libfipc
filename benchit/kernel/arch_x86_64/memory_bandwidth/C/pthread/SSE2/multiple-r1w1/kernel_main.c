 ...
 */
int ALLOCATION, HUGEPAGES, RUNS, OFFSET=0, ALIGNMENT=0, BUFFERSIZE;
int NUM_FLUSHES=0,NUM_USES=0,FLUSH_MODE,USE_MODE,NUM_THREADS=0;
int FLUSH_L1=0,FLUSH_L2=0,FLUSH_L3=0,THREAD_OFFSET=0,USE_DIRECTION;
...
void bi_cleanup( void* mdpv )
{
   ...
   if ( mdp )
   {
     ...
     if (HUGEPAGES==HUGEPAGES_ON)
     {
       if(mdp->buffer!=NULL) munmap((void*)mdp->buffer,BUFFERSIZE);unlink(filename);
     }
