...
int enqueue_forw ( queue_t* q, node_t* r )
{
	node_t* msg;
...
int dequeue_forw ( queue_t* q, data_t* data )
{
	node_t* msg;
...
int enqueue_back ( queue_t* q, node_t* r )
{
	node_t* msg;

	if (fipc_send_msg_start( q->tail, &msg ) != 0)
		return NO_MEMORY;

	msg->regs[0] = r->regs[0];
	fipc_send_msg_end ( q->tail, msg );
...
int dequeue_back ( queue_t* q, data_t* data )
{
	node_t* msg;

	if (fipc_recv_msg_start( q->head, &msg) != 0)
		return EMPTY_COLLECTION;
