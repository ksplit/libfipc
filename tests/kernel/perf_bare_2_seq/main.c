...
{
	// Write Request
	req_line.regs[0] = req_sequence.regs[0];

	// Read Response
	while ( likely(resp_line.regs[0] != req_sequence.regs[0]) )
		fipc_test_pause();
...
{
	// Read Request
	while ( likely(req_line.regs[0] != resp_sequence.regs[0] ))
		fipc_test_pause();

	// Write Response
	resp_line.regs[0] = resp_sequence.regs[0];
