...
extern unsigned long foo(unsigned long, unsigned long, unsigned long);
extern void register_chans(struct ttd_ring_channel *, struct ttd_ring_channel *);

unsigned int send_slot_avail = 0xC1346BAD;
unsigned int send_message = 0xBADBEEF;
unsigned int recv_message = 0xBADBEEF;
unsigned int recv_ack = 0xC1346BAD;
...
struct ipc_message * get_next_available_slot(struct ttd_ring_channel *chan,
...
int wait_for_producer_slot(struct ipc_message *imsg, unsigned int token)
{
...
int wait_for_consumer_slot(struct ipc_message *imsg, unsigned int token)
{
...
void send(struct ipc_message *trans)
{
...
struct ipc_message *recv(struct ttd_ring_channel *chan)
{
	struct ipc_message *recv_msg;
	recv_msg = get_next_available_slot(chan, recv_slot);
	recv_slot++;
	wait_for_consumer_slot(recv_msg, recv_message);

	/* Once AWI's are ready
	switch(msg->fn_type) {
	case FOO:
		//Walk AWI's and dispatch to that cacti stack
		break;
	case BAR:
	*/
	return recv_msg;
...
static int ipc_thread_func(void *input)
{
	...
	volatile unsigned long foo_res;
	...
	start64 = end64 = 0;
	register_chans(container->channel_tx, container->channel_rx);
	...
	while (count < NUM_LOOPS) {
		...
		foo_res = foo(start64, end64, 0x31337);
		count++;
