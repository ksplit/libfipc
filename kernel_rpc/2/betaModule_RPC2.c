...
extern unsigned long foo_dispatch(struct ipc_message *);
extern unsigned long bar_dispatch(struct ipc_message *);
extern unsigned long baz_dispatch(struct ipc_message *);
...
unsigned int recv_msg_ready = 0xBADBEEF;
unsigned int recvd_ack = 0xC1346BAD;
unsigned int send_slot_avail = 0xC1346BAD;
unsigned int send_msg_ready = 0xBADBEEF;
...
static inline void msg_status_mwait(unsigned long rcx, volatile uint32_t *rax,
				 unsigned long wait_type)
{
	...
	/*cpu = smp_processor_id();
	  ...
	  if (cpu_has_bug(&cpu_data(cpu), X86_BUG_CLFLUSH_MSG_STATUS)) {
	  mb();
	  clflush(rax);
	  mb();
	  }*/
...
{
	return (unlikely(loc->msg_status != token));// && (loc->msg_status != 0);
}
...
static int wait_for_producer_slot(struct ipc_message *imsg, unsigned int token)
{
	...
	while (unlikely(check_prod_slot_available(imsg, token))) {

#if defined(USE_MWAIT)
		msg_status_mwait(ecx, &imsg->msg_status, cstate_wait);
#endif//usemwait
#if defined(POLL)
			cpu_relax();
...
static void send(struct ipc_message *msg)
{
	msg->msg_status = send_msg_ready;
	send_slot++;
}

static struct ipc_message *recv(struct ttd_ring_channel *recv)
{

	struct ipc_message *r_msg;
	r_msg = get_next_available_slot(recv, recv_slot);
	recv_slot++;
	wait_for_consumer_slot(r_msg, recv_msg_ready);
	return r_msg;
}

static void transact(struct ttd_ring_channel *prod, struct ttd_ring_channel  *cons)
{
	struct ipc_message *recvd_msg;
	struct ipc_message *response;
	response = get_next_available_slot(prod, send_slot);
	wait_for_producer_slot(response, send_slot_avail);
	recvd_msg =  recv(cons);


	/* CURRENT IMPLEMENTATION ZERO COPIES DATA INTO THE SEND BUFFER */
	/* TEST TO SEE IF WE CAN DO THE RESPONSE ON THE SStACK THEN COPY OVER? */
	switch(recvd_msg->type) {
	case FOO:
		response->reg1 = foo_dispatch(recvd_msg);
		response->type = FOO;
		recvd_msg->msg_status = recvd_ack;
		send(response);
		break;
	case BAR:
		bar_dispatch(recvd_msg);
		break;
        case BAZ:
		baz_dispatch(recvd_msg);
		break;
	default:
		pr_err("something other than enum on cpu %d\n", CPU_NUM);
...
#endif
		transact(prod_channel, cons_channel);
		count++;
